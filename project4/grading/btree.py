...
class BTreeBlock(Block):
	...
	def insert(self, key, ptr):
		if self.isLeaf:
			...
			if self.hasSpace():
				...
				oldlist = self.keysAndPointers                 #copy the keys and pointers to "oldlist"
				self.keysAndPointers = list()                  #reset the two nodes/blocks to empty
				lprime.keysAndPointers = list()

				for i in range(0, int(len(oldlist)/4)*2): 
					self.keysAndPointers.append(oldlist[i])
				self.keysAndPointers.append(Pointer(lprime.blockNumber))     #left half stays in the old node

				for i in range(int(len(oldlist)/4)*2, len(oldlist)): 
					lprime.keysAndPointers.append(oldlist[i])                # right half goes to the new node
	...
	def redistributeWithBlock(self, otherBlock):
		#for i in range(0, len(self.keysAndPointers) + 2):
		#	self.keysAndPointers[i+2] = self.keysAndPointers[i]
		parentBlock = self.parent.getBlock()

		for i in range(0, len(parentBlock.keysAndPointers),2):
			if parentBlock.keysAndPointers[i].blockNumber == self.blockNumber:
				if i == 0:
					#print i
					if self.isUnderfull():
						self = self
						...
						otherBlock = self
						self = parentBlock.keysAndPointers[i+2].getBlock()                                     
		
		for index in range(0, len(parentBlock.keysAndPointers), 2):
			if parentBlock.keysAndPointers[index].blockNumber == self.blockNumber:
				kprime = parentBlock.keysAndPointers[index-1]
				#print kprime
		...
		if self.isLeaf:
			for i in range(0, len(parentBlock.keysAndPointers),2):
				if parentBlock.keysAndPointers[i].blockNumber == self.blockNumber:
					if i != 0:
						#print i 
						self.keysAndPointers.insert(0,otherBlock.keysAndPointers[len(otherBlock.keysAndPointers)-2])
						self.keysAndPointers.insert(0,otherBlock.keysAndPointers[len(otherBlock.keysAndPointers)-3]) 
						otherBlock.keysAndPointers.pop(len(otherBlock.keysAndPointers)-3)
						otherBlock.keysAndPointers.pop(len(otherBlock.keysAndPointers)-2)
						newKey = self.keysAndPointers[1]
					elif i == 0:
						...
						otherBlock.keysAndPointers.pop(0)
						otherBlock.keysAndPointers.pop(1)
						newKey = otherBlock.keysAndPointers[1]
		#self.keysAndPointers[0] = otherBlock.keysAndPointers[len(otherBlock.keysAndPointers)-2]
		#self.keysAndPointers[1] = otherBlock.keysAndPointers[len(otherBlock.keysAndPointers)-1]
			...
			newkey = otherBlock.keysAndPointers[len(otherBlock.keysAndPointers)-2]
			otherBlock.keysAndPointers.pop(len(otherBlock.keysAndPointers)-2)
			otherBlock.keysAndPointers.pop(len(otherBlock.keysAndPointers)-1)
		#print "newKey"
