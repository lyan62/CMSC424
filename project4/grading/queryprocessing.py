...
class GroupByAggregate(Operator):
	...
	def update_aggregate(aggregate_function, current_aggregate, new_value):
		if aggregate_function == GroupByAggregate.COUNT:
			...
			current_aggregate[0] = current_aggregate[0] + 1
			current_aggregate[1] = current_aggregate[1] + float(new_value)
	...
	def final_aggregate(aggregate_function, current_aggregate):
		if aggregate_function in [GroupByAggregate.COUNT, GroupByAggregate.SUM, GroupByAggregate.MIN, GroupByAggregate.MAX]:
			...
			return (float)(current_aggregate[1]/current_aggregate[0])
		elif aggregate_function == GroupByAggregate.MEDIAN:
			#print current_aggregate
			fl = sorted(current_aggregate)
			#print fl
...
class SortMergeJoin(Operator):
	...
	def get_next(self):
		...
		left_input = [r for r in self.left_child.get_next()]          #load relations into array(lef_input)
		right_input = [r for r in self.right_child.get_next()]        #load relations into array(right_input)

		left_input.sort(key=lambda t: t.getAttribute(self.left_attribute))   #sort arrray by attribute(e.g.dept_name)
		right_input.sort(key=lambda t: t.getAttribute(self.right_attribute))

		if self.jointype == self.INNER_JOIN:
			if self.left_child.get_next() is None or self.right_child.get_next() is None:
				output = None
				yield Tuple(None,output)
			
			ptr_l = 0   # just index like i 
			ptr_r = 0   

			while ptr_l < len(left_input) and ptr_r < len(right_input):
				set_L = [left_input[ptr_l]]              #set_L = [left_input[i]]
			...
			if self.left_child.get_next() is None and self.right_child.get_next() is None:
				output = None
				yield Tuple(None,output)
			elif self.left_child.get_next() is None:
				output = self.right_child.get_next()
				yield Tuple(None,output)
			elif self.right_child.get_next() is None:
			...
			ptr_r = 0   
			ls_r = []
			for r in right_input:
				ls_r.append(r.getAttribute(self.right_attribute))
			#print ls_r
			...
			while ptr_l < len(left_input) and ptr_r < len(right_input):
				...
				set_R = [right_input[ptr_r]]
				#print set_R
				r_attr = right_input[ptr_r].getAttribute(self.right_attribute)
						...
						if right_input[ptr_r].getAttribute(self.right_attribute) == l_attr:
							...
							for r in set_R:
								if r is None:
									...
									output = list(r.t)
									for i,attr in enumerate(left_input[ptr_l].schema):
										output.insert(i,'NULL')
									yield Tuple(None, output)
						ptr_r += 1
					
				else:
					output = list(left_input[ptr_l].t)
					for i in range(0, len(right_input[ptr_r].schema)):
						output.append("NULL")
					yield Tuple(None,output)
					ptr_l += 1
...
class SetMinus(Operator):
	...
	def get_next(self):
		if self.left_child.get_next() is None:
			output = None
			yield Tuple(None, output)
		elif self.right_child.get_next() is None:
			output = self.left_child.get_next()
		...
		if self.keep_duplicates is False:
			for r in self.right_child.get_next():
				if r not in hashtable:
					hashtable.append(r.t)
			#print hashtable
			for l in self.left_child.get_next():
				if l.t not in hashtable:
					output = list(l.t)
					yield Tuple(None, output)

		else:
			for r in self.right_child.get_next():
				hashtable_r.append(r.t)
			for l in self.left_child.get_next():
				hashtable_l.append(l.t)
			for i in hashtable_r:
				if i in hashtable_l:
					hashtable_l.remove(i)
			for l in hashtable_l:
				output = list(l)
				yield Tuple(None,output)
