...
class LockTable:
	...
	compatibility_list = [(IS, IS), (IS, S), (S, IS), (S, S), (IX,IS), (IX,IX), (IS,IX)]
	@staticmethod
	...
	def detectDeadlocksAndChooseTransactionsToAbort():
		with LockTable.hashtable_lock:
			...
			list2 = []
			trans_list = []
			current_and_waiting_list = []
			abortidlist = []
			#vertex1 = []
			#vertex2 = []
			
			for i in LockTable.lockhashtable:
				entrylist= LockTable.lockhashtable[i]
				tid = i
				#print entrylist.current_transactions_and_locks

				for i in entrylist.current_transactions_and_locks:
					list1.append((tid, i))
					#vertex1.append((tid, i[0]))
					#print "current"
					#print list1
					#print vertex1

				for j in entrylist.waiting_transactions_and_locks:
					list2.append((tid,j))
					#vertex2.append((tid,j[0]))
					#waitingtrans = j
					#print "waiting"
					#print list2
			...
			for a in list1:
				for b in list2:
					if a[0] == b[0]:
						trans_list.append((a[0],(a[1],b[1]))) 
						current_and_waiting_list.append((a[1][0],b[1][0]))

			#print "trans_list"
			#print trans_list	
			print current_and_waiting_list

			graph = {}

			if current_and_waiting_list !=[]:
				#last = len(current_and_waiting_list)
				for i in current_and_waiting_list:
					graph.update({i[1]:[i[0]]})

			final_list = cycle_exists(graph)
			print final_list
			#print graph
			if final_list !=[]:
				if len(final_list) > 1:
					for i in final_list:
						abortidlist.append(i[0])
					return abortidlist
				else:
...
class LogManager:
	...
	def restartRecovery():
		...
		at_list = []
		for i in allrecords:
			if i.info[1] == 'COMMIT' or i.info[1]=='ABORT':
				at_list.append(i.info[0])
		
		duplist = []
		for i in allrecords:
			if i.info[0] not in at_list:
				duplist.append([i.info[0], i.info[1]])
		alist = []
		#print duplist
		for i in duplist:
			#print (i)
		...
		for i in duplist:
			if i[1] !='START':
				LogManager.revertChanges(i[0])
				LogManager.createAbortLogRecord(i[0])	
			else:
				if i[0] not in alist:
					LogManager.createAbortLogRecord(i[0])
